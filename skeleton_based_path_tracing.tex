...
\end{tabular}
\caption{\label{fig::intro_image} Regardez comment les rayons s'orientent vers les parties éclairées de la scène gr\^ace à notre skelette curviligne( image $b$), comparé à l'algorithme classique du lancer de rayon (image $a$). Les images $c$ et $d$ montrent la réduction du bruit obtenu par le \emph{path tracing} classique ($c$) et notre méthode ($d$).}
\end{center}
\end{figure*}
...
La dernière étape consiste à convertir le squelette obtenu (représenté par un cubique complexe) en un graphe dont les n\oe uds sont des points de $\mathbb{R}^3$ et dont les arr\^etes encodent la topologie de la scène. 

En plus du squelette nous conservons une grille 3D couvrant la scène. On stocke dans chaque case de cette grille l'indice du n\oe d du squelette le plus proche. Ainsi nous pouvons récupérer en temps constant le n\oeu d le plus proche de chaque point 3D de la scène.

\purple{DETAILLE MIEUX, RAJOUTE L'HISTOIRE DE LA GRILLE 3D QUI NOUS PERMET D'AVOIR EN TEMPS CONSTANT ACCES pour tout point de la scène au plus proche point du squelette}

\subsection{Calcul des points d'importance}

\purple{INSERE UN SCHEMA}
...
Notre méthode conduit à une bonne réduction du bruit, en particulier dans les zones sombres ou l'illumination est exclusivement indirecte.
\purple{AJOUT D'IMAGES illustrative}
...
D'autre part, plusieurs cas pathologiques, relativement rare, peuvent nous conduire à nous rabattre sur un échantillonnage basé sur la BRDF:
\begin{itemize}
...
\end{tabular}
\caption{\label{fig::figure_correction_MIS} \purple{A ECRIRE : c'est plus sombre au milieu}}
\end{center}
\end{figure*}
