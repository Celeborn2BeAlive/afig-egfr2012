...
\purple{REINTRODUIRE OU TRADUIRE LE TROISIEME PARAGRAPHE DU PAPIER EG12}
Des travaux~\cite{Jensen95,HP02} en ce sens exploitent une carte de photon afin de construire une estimation de l'irradiance affectant un point d'intersection lors du lancer de rayon. Cette estimation est utilisée afin d'échantillonner efficacement l'hémisphère autour du point en accord avec la distribution de lumière dans la scène. Cette méthode est efficace mais peut \^etre coûteuse en espace (stockage de la carte de photons) et de plus, ne peut \^etre exploitée que si le nombre de photons est suffisant ce qui n'est pas le cas dans des zones très occultées.

\purple{S'inspirer du 4ème § du papier EG12 PUIS INSISTER SUR LE MIS (puisque c'est la chose nouvelle)}
Nous présentons une méthode d'échantillonnage pour l'algorithme du lancer de rayons \ref{Biri12}. Cette méthode est basé sur l'utilisation d'un squelette topologique curviligne du vide de la scène. Il nous permet de choisir des directions porteuses d'énergie lors d'un rebond et ainsi améliorer la vitesse de convergence de l'algorithme. La figure \ref{fig::intro_image} illustre la distribution des rayons dans la scène et la réduction du bruit obtenue avec notre algorithme. Nous introduisons ensuite une amélioration de la méthode en utilisant une stratégie d'échantillonnage préférentiel multiple combinant notre densité de probabilité et une densité en accord avec le modèle de réflexion utilisé. Cette stratégie nous permet d'éliminer certains artefacts produit par la méthode originale et d'obtenir de meilleurs résultats.
...
\begin{figure}[t!]
\begin{center}
\myincludegraphics{0.2\textwidth}{rendering_equation.png}
...
\end{tabular}
\caption{\label{fig::figure_correction_MIS} Notre méthode (image de gauche) sans MIS conduit à rater des échanges lumineux entre surfaces proche. Le path tracing standard (image de droite) génère un résultat plus sombre du à l'occlusion ambiante dans le coin.}
\end{center}
\end{figure*}
